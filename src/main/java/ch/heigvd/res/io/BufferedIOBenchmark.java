...
public class BufferedIOBenchmark {
	...
	private static CsvFile csv;// objet pour le fichier csv
	...
	private void produceTestData(IOStrategy ioStrategy, long numberOfBytesToWrite, int blockSize) {
		...
		// Ecriture des donnes pour l'ecriture
		csv.writeData("\nWRITE," + ioStrategy + "," + blockSize + "," + NUMBER_OF_BYTES_TO_WRITE + "," + time);
	...
	private void consumeTestData(IOStrategy ioStrategy, int blockSize) {
		...
		long time = Timer.takeTime();
		LOG.log(Level.INFO, "  > Done in {0} ms.", time);
		// Ecriture des donnees poue la lecture
		csv.writeData("\nREAD," + ioStrategy + "," + blockSize + "," + NUMBER_OF_BYTES_TO_WRITE + "," + time);
...
class CsvFile
{
	...
	// Ouverture du fichier de sortie csv et ecriture de l'entete
	CsvFile(String fileName)
	{
		try
		{
			out = new FileWriter(fileName);
			out.write("operation,strategy,blockSize,fileSizeInBytes,durationInMs");
		}catch (IOException e)
		{
			e.printStackTrace();
		}
	...
	// Ecriture dans le fichier du string passe en parametre
